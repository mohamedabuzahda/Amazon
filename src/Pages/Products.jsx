...
const initialProducts = [
  ...
  },
  {
    ...
    image: "/src/assets/images/card7.png",
    rating: 4.3,
    reviews: 10122,
    originalPrice: 23990,
    currentPrice: 16490,
  ...
  },
  {
    ...
    image: "/src/assets/images/card8.png",
    rating: 4.1,
    reviews: 1440,
    originalPrice: 18990,
    currentPrice: 12990,
  ...
  },
  {
    ...
    image: "/src/assets/images/card9.png",
    rating: 4.2,
    reviews: 13204,
    originalPrice: 16990,
    currentPrice: 12990,
  ...
  },
  {
    ...
    image: "/src/assets/images/card10.png",
    rating: 4.2,
    reviews: 610,
    originalPrice: 54990,
    currentPrice: 32990,
  ...
  },
  {
    ...
    image: "/src/assets/images/card11.png",
    rating: 4.2,
    reviews: 1522,
    originalPrice: 18990,
    currentPrice: 13490,
  ...
  },
  {
    ...
    image: "/src/assets/images/card12.png",
    rating: 4.3,
    reviews: 25505,
    originalPrice: 26990,
    currentPrice: 19990,
  ...
  useEffect(() => {
    ...
    if (activeFilters.delivery) {
      filteredProducts = filteredProducts.filter(product => {
        ...
        const daysDifference = Math.ceil((deliveryDate - today) / (1000 * 60 * 60 * 24))
    ...
    if (activeFilters.priceRange.length > 0) {
      filteredProducts = filteredProducts.filter(product => {
        return activeFilters.priceRange.some(range => {
          switch (range) {
            case '10000-15000':
              return product.currentPrice >= 10000 && product.currentPrice <= 15000
            case '15000-20000':
              return product.currentPrice >= 15000 && product.currentPrice <= 20000
            case '20000-35000':
              return product.currentPrice >= 20000 && product.currentPrice <= 35000
            case '35000+':
          ...
          <div className={`w-full lg:w-64 flex-shrink-0 ${showFilters ? 'block' : 'hidden lg:block'}`}>
            <div className="bg-white shadow-sm p-4 space-y-6 rounded-lg lg:rounded-none">
              ...
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center">
                    ...
                    />
                    <span className="ml-2 text-sm text-gray-700">Get it in 2 Days</span>
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {['Samsung', 'LG', 'Whirlpool', 'Godrej', 'Panasonic', 'Fica'].map((brand) => (
                ...
                <div className="space-y-2">
                  <label className="flex items-center">
                    <input 
                      ...
                      className="w-4 h-4 text-blue-600 rounded" 
                    />
                    <span className="ml-2 text-sm text-gray-700">₹10,000 to ₹15,000</span>
                  </label>
                  <label className="flex items-center">
                    <input 
                      type="checkbox" 
                      ...
                      className="w-4 h-4 text-blue-600 rounded" 
                    />
                    <span className="ml-2 text-sm text-gray-700">₹15,000 to ₹20,000</span>
                  </label>
                  <label className="flex items-center">
                    <input 
                      type="checkbox" 
                      ...
                      className="w-4 h-4 text-blue-600 rounded" 
                    />
                    <span className="ml-2 text-sm text-gray-700">₹20,000 to ₹35,000</span>
                  </label>
                  <label className="flex items-center">
                    <input 
                      type="checkbox" 
                      ...
                      className="w-4 h-4 text-blue-600 rounded" 
                    />
                    <span className="ml-2 text-sm text-gray-700">₹35,000 & Above</span>
                  </label>
                </div>
              </div>

              {/* Active Filters Display */}
              {(activeFilters.delivery || activeFilters.rating || activeFilters.brands.length > 0 || activeFilters.priceRange.length > 0) && (
                <div className="pt-4 border-t">
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    {activeFilters.brands.map(brand => (
                      <div key={brand} className="text-blue-600">• {brand}</div>
                    ))}
                    {activeFilters.priceRange.map(range => (
                      <div key={range} className="text-blue-600">
                        • {range === '35000+' ? '₹35,000+' : `₹${range.replace('-', ' - ₹')}`}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {products.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                {products.map((product) => (
                  <div key={product.id} className="bg-white shadow-sm hover:shadow-md transition-shadow duration-200 p-3 sm:p-4 border rounded-lg">
                    {/* Product Image */}
